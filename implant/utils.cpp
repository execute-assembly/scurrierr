...
BOOL ParseCommandJson(PBYTE CommandString) {
	...
	*/
	...
	g_Bytes->index = 0;
	UINT ErrorCodeCheck = g_Bytes->Read4(CommandString);
	if (ErrorCodeCheck > 0) {
	...
	CommandInfo->commandID = g_Bytes->Read4(CommandString);
	CommandInfo->commandCode = g_Bytes->Read4(CommandString);
	UINT Param1Len = g_Bytes->Read4(CommandString);
	if (Param1Len <= 0) return TRUE;

	if (Param1Len < MAX_PARAM_LENGTH) {
		CHAR ParamBuffer[MAX_PARAM_LENGTH];

		g_Bytes->ReadBytes(CommandString, (PBYTE)ParamBuffer, Param1Len);
	...
	UINT Param2Len = g_Bytes->Read4(CommandString);
	if (Param2Len <= 0) return TRUE;
	if (Param2Len < MAX_PARAM_LENGTH) {
		CHAR Param2Buffer[MAX_PARAM_LENGTH];

		g_Bytes->ReadBytes(CommandString, (PBYTE)Param2Buffer, Param2Len);
...
BOOL ParseCommand(BYTE* data) {
	...
	UINT ErrorCode = g_Bytes->Read4(data);

	CommandInfo->commandID = g_Bytes->Read4(data);
	printf("Code: %d\n", CommandInfo->commandID);
	CommandInfo->commandCode = g_Bytes->Read4(data);
	printf("ID: %d\n", CommandInfo->commandCode);

	UINT Param1Length = g_Bytes->Read4(data);
	PCHAR Str1 = AllocMemory<CHAR>(Param1Length);
	g_Bytes->ReadBytes(data, (BYTE*)Str1, Param1Length);
	CommandInfo->param = _strdup(Str1);
	printf("param 1: %s\n", CommandInfo->param);
	HeapFree(GetProcessHeap(), 0, Str1);

	UINT Param2Len = g_Bytes->Read4(data);

	PCHAR Str2 = AllocMemory<CHAR>(Param2Len);
	g_Bytes->ReadBytes(data, (BYTE*)Str2, Param2Len);
